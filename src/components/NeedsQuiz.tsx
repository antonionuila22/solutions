...
import {
  ...
  SCORING,
  type ResultType,
  type ResultContentData,
} from "../data/quiz-data";

interface ResultContentWithIcon extends ResultContentData {
  icon: ReactNode;
}

const resultIcons: Record<NonNullable<ResultType>, ReactNode> = {
  website: (
    <svg
...
export default function NeedsQuiz() {
  ...
  const getResult = (): ResultType => {
    ...
    if (websitePercent >= SCORING.WEBSITE_THRESHOLD) {
      return "website";
    }

    if (ecommercePercent >= SCORING.ECOMMERCE_THRESHOLD) {
      if (webapp >= SCORING.ADVANCED_ECOMMERCE_SCORE) {
        return "ecommerce-advanced";
      }
      return "ecommerce-basic";
    }

    if (webappPercent >= SCORING.WEBAPP_THRESHOLD) {
      if (webapp >= SCORING.ADVANCED_WEBAPP_SCORE) {
        return "enterprise";
      }
      return "webapp";
    }
  ...
  if (showResult && result) {
    ...
    const content: ResultContentWithIcon = {
      ...data,
      icon: resultIcons[result],
    };
    const colorClasses = {
      cyan: {
        bg: "bg-cyan-500/20",
